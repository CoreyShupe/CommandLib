...
public class CommandFactory<I> {
	...
	private void registerCommand(Object classInstance, Method method, Predicate<I> permissionChecker, Command annotation) {
		...
		for (int i = (annotation.ignoreAuthor() ? 0 : 1), x = 0, z = 0; i < params.length - 1; i++, x++) {
		...
		registerCommand(
				AbstractCommand.newCommand(
						authorClass, annotation, permissionChecker, (author, content) -> {
